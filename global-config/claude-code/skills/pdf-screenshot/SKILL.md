...
Claude's built-in PDF reader extracts text, but loses visual layout — charts, diagrams, formatted tables, images, and multi-column layouts. This skill renders PDF pages as PNG images so Claude's vision can analyze the actual visual content.
...
Use the Read tool on the generated PNG files. Claude's vision will analyze the visual content including:
...
- PDF is text-only with simple formatting — use Claude's built-in PDF reader instead
